...
{"ERR_PGMODELER_HERANCATABINV", QT_TR_NOOP("Is not possible to create a self generalization/copy relationship! The table can not inherit or copy their own attributes!")},
...
{"ERR_PGMODELER_ATRPKEXISTETAB", QT_TR_NOOP("Assignment of a primary key to a table which already has one!")},
 {"ERR_PGMODELER_RELIDENTINV", QT_TR_NOOP("Identifier relationship can not be created for a self relationship, relationships of the type n-n, copy or generalization!")},
 {"ERR_PGMODELER_RELINVCOLDUPLIC", QT_TR_NOOP("Unable to create a copy relationship because the column '%1' in table '%2' already exists in table '%3'!")},
...
{"ERR_PGMODELER_ATROBJRELINV", QT_TR_NOOP("An attribute can not be added to a copy or generalization relationship!")},
...
{"ERR_PGMODELER_INSRELGERAREDUNDANCIA", QT_TR_NOOP("The configuration of the relationship '%1' generates a redundancy between the relationships '%2'. Redundancy on identifier or generalization/copy relationships are not accepted since they result in  incorrect column spreading making the model inconsistent!")},
...
{"ERR_PGMODELER_PKESPTIPORELINV", QT_TR_NOOP("The relationship '%1' can not make use of the special primary key. Only generalization/copy relationships have access to this resource!")},
