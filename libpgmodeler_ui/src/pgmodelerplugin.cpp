...
PgModelerPlugin::PgModelerPlugin(void)
{
 ...
 connect(plugin_info_frm->aplicar_ok_btn, SIGNAL(clicked(void)), plugin_info_frm, SLOT(close(void)));
 ...
 plugin_info_frm->setWindowTitle(trUtf8("Plugin Information"));
 ...
 title_lbl->setText("Plugin title");
                                          ...
                                          const QString &description, const QString &ico_filename)
{
 ...
 version_lbl->setText(trUtf8("Version: %1").arg(version));
 author_lbl->setText(trUtf8("Author: %1").arg(author));
 description_lbl->setText(description);
