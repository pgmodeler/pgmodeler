...
void ModelWidget::alignTablesTop(void)
{
	double ty=-1;
	BaseTableView *tab;
	...
	for(auto &obj : selected_objects)
	{
		...
		{
			tab=dynamic_cast<BaseTableView *>(dynamic_cast<BaseGraphicObject *>(obj)->getOverlyingObject());
			tabs.push_back(tab);
...
void ModelWidget::alignTablesVerticalCenter(void)
{
	double ty=-1, by=-1;
	BaseTableView *tab;
	vector<BaseTableView*> tabs;
...
void ModelWidget::alignTablesBottom(void)
{
	double by=-1;
	BaseTableView *tab;
	vector<BaseTableView*> tabs;
...
void ModelWidget::distributeTablesVertically(void)
{
	...
	for(auto ii=tabs.begin(), prev=tabs.end(); ii!=tabs.end(); prev=ii, ++ii)
	{
...
void ModelWidget::alignTablesLeft(void)
{
	double lx=-1;
	BaseTableView *tab;
	vector<BaseTableView*> tabs;
...
void ModelWidget::alignTablesHorizontalCenter(void)
{
	double lx=-1, rx=-1;
	BaseTableView *tab;
	vector<BaseTableView*> tabs;
...
void ModelWidget::alignTablesRight(void)
{
	double rx=-1;
	BaseTableView *tab;
	vector<BaseTableView*> tabs;
...
void ModelWidget::distributeTablesHorizontally(void)
{
...
bool ModelWidget::stackTablesOnTop()
{
	bool any_zvalue_changed=false;

	for(auto obj:selected_objects)
		if(obj->getObjectType()==ObjectType::Table || obj->getObjectType()==ObjectType::View)
	{
		auto tab=dynamic_cast<BaseTableView *>(dynamic_cast<BaseGraphicObject *>(obj)->getOverlyingObject());
		if(!tab->collidingItems().empty())
		{
			int z_value=tab->zValue();
			auto items=tab->collidingItems();
			for(const auto &item : items)
				if(z_value< item->zValue())
					z_value=item->zValue();
			tab->setZValue(z_value+1);
			any_zvalue_changed=true;
		}
	}
	return any_zvalue_changed;
}
