...
void FormExportacao::exportarModelo(void)
{
 try
 {
  ...
  if(exportacao_png_rb->isChecked())
  {
   ...
   ObjectsScene::obterOpcoesGrade(exibir_grade, alin_objs, exibir_lim);

   //Redefine as opções da grade conforme os checkboxes
   ObjectsScene::definirOpcoesGrade(grade_chk->isChecked(), false, lim_paginas_chk->isChecked());
   ...
   ObjectsScene::definirOpcoesGrade(exibir_grade, alin_objs, exibir_lim);
   modelo_wgt->cena->update();
