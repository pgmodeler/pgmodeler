...
void BaseObjectWidget::configureTabOrder(vector<QWidget *> widgets)
{
  ...
  vector<QWidget *> tab_order, children;
  int idx=0, cnt=0;

  widgets.insert(widgets.begin(),
                { name_edt, schema_sel, tablespace_sel,
                  owner_sel, collation_sel, comment_edt,
                  append_sql_tb, edt_perms_tb, disable_sql_chk });
