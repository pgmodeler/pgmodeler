   ...
   for(id_tipo=0; id_tipo < qtd_tipos; id_tipo++)
   {
    ...
    for(id_lin=0; map_objs_visiveis[tipos[id_tipo]] && id_lin < qtd; id_lin++)
    {
     ...
     if(objeto->isProtected())
     {
      ...
      item_tab->setForeground(ObjetoGrafico::obterEstiloFonte(ParsersAttributes::PROT_COLUMN).foreground());
     }
    ...
    for(id_tipo=0; id_tipo < qtd_subtipos; id_tipo++)
    {
     ...
     for(id_lin=0; map_objs_visiveis[subtipos[id_tipo]] && id_lin < qtd1; id_lin++)
     {
      ...
      if(objeto_tab->isAddedByRelationship())
      {
       ...
       fonte.setItalic(true);
       item_tab->setFont(fonte);
       item_tab->setForeground(ObjetoGrafico::obterEstiloFonte(ParsersAttributes::INH_COLUMN).foreground());
      }
      /* Caso o objeto esteja protegido, configura um estilo de fonte para
        indicar esta situação */
  ...
  try
  {
   ...
   for(i=0; i < qtd; i++)
   {
    ...
    if(esquema && esquema->isProtected())
    {
     ...
     item2->setForeground(0,ObjetoGrafico::obterEstiloFonte(ParsersAttributes::PROT_COLUMN).foreground());
    }
     ...
     if(map_objs_visiveis[tipos[i1]])
     {
      ...
      for(i2=0; i2 < qtd2; i2++)
      {
       ...
       if(objeto->isProtected())
       {
        ...
        item4->setForeground(0,ObjetoGrafico::obterEstiloFonte(ParsersAttributes::PROT_COLUMN).foreground());
       }
  ...
  try
  {
   ...
   for(i=0; i < qtd; i++)
   {
    ...
    if(tabela->isProtected())
    {
     ...
     item1->setForeground(0,ObjetoGrafico::obterEstiloFonte(ParsersAttributes::PROT_COLUMN).foreground());
    }
     ...
     if(map_objs_visiveis[tipos[i1]])
     {
      ...
      for(i2=0; i2 < qtd1; i2++)
      {
       ...
       if(dynamic_cast<TableObject *>(objeto)->isAddedByRelationship())
       {
        ...
        fonte.setItalic(true);
        item3->setFont(0,fonte);
        item3->setForeground(0,ObjetoGrafico::obterEstiloFonte(ParsersAttributes::INH_COLUMN).foreground());
       }
       /* Caso o objeto esteja protegido, configura um estilo de fonte para
          indicar esta situação */
       else if(objeto->isProtected())
       {
        ...
        fonte.setItalic(true);
        item3->setFont(0,fonte);
        item3->setForeground(0,ObjetoGrafico::obterEstiloFonte(ParsersAttributes::PROT_COLUMN).foreground());
       }
  ...
  try
  {
   ...
   if(map_objs_visiveis[OBJ_DATABASE])
   {
    ...
    if(/*modelo_wgt->*/modelo_bd->isProtected())
    {
     ...
     raiz->setForeground(0,ObjetoGrafico::obterEstiloFonte(ParsersAttributes::PROT_COLUMN).foreground());
    }
