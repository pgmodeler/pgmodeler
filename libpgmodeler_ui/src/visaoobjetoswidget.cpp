   ...
   for(id_tipo=0; id_tipo < qtd_tipos; id_tipo++)
   {
    ...
    for(id_lin=0; map_objs_visiveis[tipos[id_tipo]] && id_lin < qtd; id_lin++)
    {
     ...
     if(objeto->objetoProtegido())
     {
      ...
      item_tab->setForeground(ObjetoGrafico::obterEstiloFonte(ParsersAttributes::COLUNA_PROTEGIDA).foreground());
     }
    ...
    for(id_tipo=0; id_tipo < qtd_subtipos; id_tipo++)
    {
     ...
     for(id_lin=0; map_objs_visiveis[subtipos[id_tipo]] && id_lin < qtd1; id_lin++)
     {
      ...
      if(objeto_tab->incluidoPorRelacionamento())
      {
       ...
       fonte.setItalic(true);
       item_tab->setFont(fonte);
       item_tab->setForeground(ObjetoGrafico::obterEstiloFonte(ParsersAttributes::COLUNA_HERDADA).foreground());
      }
      /* Caso o objeto esteja protegido, configura um estilo de fonte para
        indicar esta situação */
  ...
  try
  {
   ...
   for(i=0; i < qtd; i++)
   {
    ...
    if(esquema && esquema->objetoProtegido())
    {
     ...
     item2->setForeground(0,ObjetoGrafico::obterEstiloFonte(ParsersAttributes::COLUNA_PROTEGIDA).foreground());
    }
     ...
     if(map_objs_visiveis[tipos[i1]])
     {
      ...
      for(i2=0; i2 < qtd2; i2++)
      {
       ...
       if(objeto->objetoProtegido())
       {
        ...
        item4->setForeground(0,ObjetoGrafico::obterEstiloFonte(ParsersAttributes::COLUNA_PROTEGIDA).foreground());
       }
  ...
  try
  {
   ...
   for(i=0; i < qtd; i++)
   {
    ...
    if(tabela->objetoProtegido())
    {
     ...
     item1->setForeground(0,ObjetoGrafico::obterEstiloFonte(ParsersAttributes::COLUNA_PROTEGIDA).foreground());
    }
     ...
     if(map_objs_visiveis[tipos[i1]])
     {
      ...
      for(i2=0; i2 < qtd1; i2++)
      {
       ...
       if(dynamic_cast<ObjetoTabela *>(objeto)->incluidoPorRelacionamento())
       {
        ...
        fonte.setItalic(true);
        item3->setFont(0,fonte);
        item3->setForeground(0,ObjetoGrafico::obterEstiloFonte(ParsersAttributes::COLUNA_HERDADA).foreground());
       }
       /* Caso o objeto esteja protegido, configura um estilo de fonte para
          indicar esta situação */
       else if(objeto->objetoProtegido())
       {
        ...
        fonte.setItalic(true);
        item3->setFont(0,fonte);
        item3->setForeground(0,ObjetoGrafico::obterEstiloFonte(ParsersAttributes::COLUNA_PROTEGIDA).foreground());
       }
  ...
  try
  {
   ...
   if(map_objs_visiveis[OBJETO_BANCO_DADOS])
   {
    ...
    if(/*modelo_wgt->*/modelo_bd->objetoProtegido())
    {
     ...
     raiz->setForeground(0,ObjetoGrafico::obterEstiloFonte(ParsersAttributes::COLUNA_PROTEGIDA).foreground());
    }
