...
CodigoFonteWidget::CodigoFonteWidget(QWidget *parent): BaseObjectWidget(parent)
{
 try
 {
...
void CodigoFonteWidget::gerarCodigoFonteObjeto(int)
{
 ...
 try
 {
  ...
  if(tipo_obj!=BASE_RELATIONSHIP && tipo_obj!=OBJ_TEXTBOX)
  {
   ...
   codigosql_txt->setPlainText(QString::fromUtf8(DatabaseModel::validateObjectDefinition(object, SchemaParser::SQL_DEFINITION)));
  }
  ...
  codigoxml_txt->setPlainText(QString::fromUtf8(DatabaseModel::validateObjectDefinition(object, SchemaParser::XML_DEFINITION)));
...
void CodigoFonteWidget::setAttributes(DatabaseModel *modelo, BaseObject *objeto)
{
 ...
 if(objeto)
 {
  ...
  try
  {
   ...
   BaseObjectWidget::setAttributes(modelo, NULL, objeto);
   ...
   obj_icon_lbl->setPixmap(icone_obj);

   //Preenche os campos do formulario com os atributos do objeto
   comment_edt->setText(objeto->getTypeName());
