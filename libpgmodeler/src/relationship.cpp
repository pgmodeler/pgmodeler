	...
	BaseRelationship(rel_type, src_tab, dst_tab, src_mdtry, dst_mdtry)
{
	try
	{
		...
		if(((rel_type==Relationship11 || rel_type==Relationship1n) &&
			...
			throw Exception(Exception::getErrorMessage(InvLinkTablesNoPrimaryKey)
							.arg(obj_name)
							.arg(src_tab->getName(true))
							.arg(dst_tab->getName(true)),
							InvLinkTablesNoPrimaryKey,__PRETTY_FUNCTION__,__FILE__,__LINE__);
		...
		if(rel_type==RelationshipDep && src_tab->getCopyTable())
			throw Exception(Exception::getErrorMessage(InvCopyRelTableDefined)
							.arg(src_tab->getName(true))
							.arg(dst_tab->getName(true))
							.arg(src_tab->getCopyTable()->getName(true)),
							InvCopyRelTableDefined,__PRETTY_FUNCTION__,__FILE__,__LINE__);
		...
		if(rel_type == RelationshipPart && !dst_tab->isPartitioned())
			throw Exception(Exception::getErrorMessage(InvPartitioningTypePartRel)
							.arg(src_tab->getSignature()).arg(dst_tab->getSignature()),
							InvPartitioningTypePartRel, __PRETTY_FUNCTION__,__FILE__,__LINE__);
		...
		if(rel_type==RelationshipPart && src_tab->getPartitionedTable())
			throw Exception(Exception::getErrorMessage(InvPartRelPartitionedDefined)
							.arg(src_tab->getName(true))
							.arg(dst_tab->getName(true))
							.arg(src_tab->getPartitionedTable()->getName(true)),
							InvPartRelPartitionedDefined,__PRETTY_FUNCTION__,__FILE__,__LINE__);
		...
		if((rel_type == RelationshipNn && (src_tab->isPartitioned() || dst_tab->isPartitioned())) ||
			...
			throw Exception(Exception::getErrorMessage(InvRelTypeForPatitionTables)
							.arg(src_tab->getName(true))
							.arg(dst_tab->getName(true))
							.arg(src_tab->isPartitioned() || src_tab->isPartition() ? src_tab->getName(true) : dst_tab->getName(true)),
							InvRelTypeForPatitionTables,__PRETTY_FUNCTION__,__FILE__,__LINE__);
...
void Relationship::setNamePattern(unsigned pat_id, const QString &pattern)
{
	if(!pattern.isEmpty())
	{
		...
		if(pat_id > PK_COL_PATTERN)
			...
			throw Exception(Exception::getErrorMessage(AsgInvalidNamePattern)
							.arg(this->getName()),__PRETTY_FUNCTION__,__FILE__,__LINE__);
...
void Relationship::setIdentifier(bool value)
{
	...
	if(value &&
		...
		throw Exception(InvIdentifierRelationship,__PRETTY_FUNCTION__,__FILE__,__LINE__);
...
void Relationship::setSpecialPrimaryKeyCols(vector<unsigned> &cols)
{
	...
	if(!cols.empty() && (isSelfRelationship() || isIdentifier()))
		throw Exception(Exception::getErrorMessage(InvUseSpecialPrimaryKey)
						.arg(this->getName()),
						InvUseSpecialPrimaryKey,__PRETTY_FUNCTION__,__FILE__,__LINE__);
...
void Relationship::setTableNameRelNN(const QString &name)
{
	if(rel_type==RelationshipNn)
	{
		if(!BaseObject::isValidName(name))
			throw Exception(AsgInvalidNameTableRelNN, __PRETTY_FUNCTION__,__FILE__,__LINE__);
...
void Relationship::addObject(TableObject *tab_obj, int obj_idx)
{
	...
	if((rel_type==RelationshipGen ||
			...
			!(tab_obj->isAddedByRelationship() &&
			  tab_obj->isProtected() &&
			  tab_obj->getObjectType()==ObjConstraint))
		throw Exception(AsgObjectInvalidRelationshipType,__PRETTY_FUNCTION__,__FILE__,__LINE__);
				...
				getObjectIndex(tab_obj) < 0)
		{
			...
			else
			{
				...
				if(rest->getConstraintType()==ConstraintType::foreign_key)
					throw Exception(AsgForeignKeyRelationship,__PRETTY_FUNCTION__,__FILE__,__LINE__);
...
{
	removeObject(attrib_idx, ObjColumn);
}

void Relationship::removeConstraint(unsigned constr_idx)
{
	removeObject(constr_idx, ObjConstraint);
}
...
{
	return(dynamic_cast<Column *>(getObject(name,ObjColumn)));
}
...
{
	return(dynamic_cast<Constraint *>(getObject(name,ObjConstraint)));
}
...
void Relationship::addConstraints(Table *recv_tab)
{
	...
	try
	{
		...
		for(constr_id=0; constr_id < constr_cnt; constr_id++)
		{
			...
			{
				constr->setName(PgModelerNs::generateUniqueName(constr, (*recv_tab->getObjectList(ObjConstraint))));
				...
				the type incompatibility of the columns involved, otherwise they are merged. */
					for(id_tab=0; id_tab < 2; id_tab++)
					{
						...
						for(i2=0; i2 < 2; i2++)
						{
							...
							if(types[i2]==ObjTable)
							{
								tab_count=aux_tab->getObjectCount(ObjTable);
								for(idx=0; idx < tab_count; idx++)
								{
									parent_tab=dynamic_cast<Table *>(aux_tab->getObject(idx, ObjTable));
...
void Relationship::addConstraintsRelGenPart(void)
{
	...
	vector<TableObject *> *constrs=parent_tab->getObjectList(ObjConstraint);
	...
	try
	{
		for(auto &obj : *constrs)
		{
			...
			{
				aux_constr=dynamic_cast<Constraint *>(child_tab->getObject(constr->getName(), ObjConstraint));
				...
				if(!aux_constr)
				{
					...
					throw Exception(Exception::getErrorMessage(InvInheritRelationshipIncompConstrs)
									.arg(constr->getName()).arg(parent_tab->getName(false, true))
									.arg(aux_constr->getName()).arg(child_tab->getName(false, true)),
									InvInheritRelationshipIncompConstrs,__PRETTY_FUNCTION__,__FILE__,__LINE__);
			}
		}
	}
	catch(Exception &e)
...
void Relationship::addForeignKey(Table *ref_tab, Table *recv_tab, ActionType del_act, ActionType upd_act)
{
	...
	try
	{
		...
		fk->setName(PgModelerNs::generateUniqueName(fk, (*recv_tab->getObjectList(ObjConstraint))));
...
void Relationship::addAttributes(Table *recv_tab)
{
	...
	try
	{
		...
		for(i=0; i < count; i++)
		{
			...
			column->setName(PgModelerNs::generateUniqueName(column, (*recv_tab->getObjectList(ObjColumn))));
...
void Relationship::copyColumns(Table *ref_tab, Table *recv_tab, bool not_null, bool is_dst_table)
{
	...
	try
	{
		...
		for(i=0; i < count; i++)
		{
			...
			column->setName(name);
			name=PgModelerNs::generateUniqueName(column, (*recv_tab->getObjectList(ObjColumn)));
			column->setName(name);
...
QString Relationship::getCodeDefinition(unsigned def_type)
{
	...
	else
	{
		...
		attributes[ParsersAttributes::RELATIONSHIP_GEN]=(rel_type==RelationshipGen ? ParsersAttributes::_TRUE_ : QString());
