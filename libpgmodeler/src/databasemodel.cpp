...
void DatabaseModel::__getObjectDependencies(BaseObject *object, vector<BaseObject *> &objs, bool inc_children)
{
  ...
  if(table)
  {
    ...
    {
      objs.push_back(table->getCopyTable());
      __getObjectDependencies(table->getCopyTable(), dep_objs, true);

      for(BaseObject *obj : dep_objs)
      {
    ...
    {
      objs.push_back(table->getAncestorTable(i));
      __getObjectDependencies(table->getAncestorTable(i), dep_objs, true);
  ...
  if(inc_children && (obj_type==OBJ_TABLE || obj_type==OBJ_VIEW || obj_type==OBJ_SCHEMA))
  {
    ...
    if(obj_type==OBJ_SCHEMA)
    {
      ...
      {
        __getObjectDependencies(aux_obj, dep_objs, true);

        for(BaseObject *obj : dep_objs)
        {
          //Tables will be included as dependency only if they are ancestor / copy of tables on the same schema
          if(obj->getObjectType()!=OBJ_TABLE ||
             (obj->getObjectType()==OBJ_TABLE &&
              aux_obj->getObjectType()==OBJ_TABLE &&
              (dynamic_cast<Table *>(aux_obj)->getObjectIndex(obj->getName(true), OBJ_TABLE) >= 0 ||
               dynamic_cast<Table *>(aux_obj)->getCopyTable() == obj)))
            objs.push_back(obj);
        }
      }
    }
        ...
        if((!constr && child->getObjectType()!=OBJ_COLUMN) ||
            (constr &&
             ((constr->getConstraintType()==ConstraintType::foreign_key &&
               constr->getReferencedTable()->getSchema()==object->getSchema()) ||
              (constr->getConstraintType()!=ConstraintType::foreign_key &&
               constr->getConstraintType()!=ConstraintType::primary_key &&
               constr->isReferRelationshipAddedColumn()))))
        {
          //objs.push_back(child);
          __getObjectDependencies(child, objs, true);
        }
      }
    }
...
vector<BaseObject *> DatabaseModel::getCreationOrder(BaseObject *object, bool inc_children)
{
  ...
  __getObjectDependencies(object, objs, inc_children);
  ...
  if(inc_children && object->getObjectType()==OBJ_SCHEMA)
  {
    for(BaseObject *obj : relationships)
    {
      ...
      {
        __getObjectDependencies(rel->getGeneratedTable(), objs, true);
      }
    }
  }
