...
class DatabaseModel:  public QObject, public BaseObject {
  ...
  public:
    ...
    /*! brief Returns a list containig all the object need to create the 'object' in the proper order.
        This method automatically includes the permissions related to the object. If the object is a schema and
        'inc_children' is set then tables generated by many-to-many relationships will be included on the returned list
        only if their schema is the same as 'object' */
    vector<BaseObject *> getCreationOrder(BaseObject *object, bool inc_children);
    ...
    /*! brief Recursive version of getObjectDependencies. Returns all the dependencies of the specified object but
        additionally its possible to include children objects (for schemas, tables or views) by using 'inc_children' param.
        This method is less efficient than the non recursive version and is used only as an auxiliary operation for
        getCreationOrder(BaseObject *object, bool inc_children) */
    void __getObjectDependencies(BaseObject *object, vector<BaseObject *> &objs, bool inc_children);
