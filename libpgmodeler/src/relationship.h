...
class Relationship: public BaseRelationship {
 private:
  /** @details Indicates that the relationship invalid because one or more critical attributes
     where modified needing to be revalidated */
  bool invalidated,

       /** @details Indicates that the column suffixes must be created automaticaly based
          on the envolved table names */
       auto_suffix;

  /** @details Stores the number of columns which were rejected at the time of relationship
  ...
  unsigned rejected_col_count;
   ...
   /** @details Stores the columns which are created to be referenced by foreign keys
      in order to represent the relationship */
  vector<Column *> gen_columns,

     /** @details Stores the primary key columns which from the referenced table
        This vector is used by the relationship validation method. */
     pk_columns;

  /** @details Stores the suffixes generated for the columns created by the relationship.
   ...
   /** @details Stores the names of the columns referenced earlier, where
  ...
  /// @details Relationship attributes (used as columns on the receiver table)
  vector<TableObject *> rel_attributes;

  /// @details Relationship constraint (used as constraints on the receiver table)
  vector<TableObject *> rel_constraints;

  /// @details Foreign key that represents the 1-n relationship
  Constraint *fk_rel1n,

             /** @details Stores reference to the primary key automatically created when the
                relationship is identifier and the weak entity does not have
                a primary key */
             *pk_relident,

             /** @details Stores the reference to the special primary key. This constraint is only available
                to generalization / copy relationships */
             *pk_special,

             /// @details Stores the unique key that represents the 1-1 relationship (including the fk_rel1n)
             *uq_rel11;

  /// @details Table created by the relationship n-n
  Table *table_relnn;

  /// @details Tables suffixes used to create unique names for the created columns
  QString src_suffix, dst_suffix;

  /// @details Indicates if the relationship is identifier (when there is a strong and a weak entity)
  bool identifier;

  /// @details Indicates if the foreign key (for 1-1, 1-n relationships) is deferrable
  bool deferrable;

  /// @details Deferral type used by the foreign key when this is deferrable
  DeferralType deferral_type;

  /** @details This vector allows the user to define which columns inherited / copied (via its indexes)
     will be used on the special primary key in the receiver table */
  vector<unsigned> column_ids_pk_rel;

  /// @details Name assigned to the generated table on n-n relationships
  QString tab_name_relnn;

  /// @details Indicates if the column exists on the referenced column list
  bool isColumnExists(Column *column);

  /// @details Executes the column addition for 1-1 relationship
  void addColumnsRel11(void);

  /// @details Executes the column addition for 1-n relationship
  void addColumnsRel1n(void);

  /// @details Executes the column addition for n-n relationship
  void addColumnsRelNn(void);

  /** @details Copy columns from one table to another. This operation is done in
  ...
  /** @details Creates the foreign key that represents the relationship and adds it
  ...
  /** @details Creates the unique key that represents the 1-1 relationship e adds it to
     the receiver table */
  void addUniqueKey(Table *ref_tab, Table *recv_tab);

  /// @details Adds the relationship attributes (columns) into receiver table
  void addAttributes(Table *recv_tab);

  /** @details Adds relationship constraints on the receiver table. If the relationship is
  ...
  /** @details Executes adicional configurations on receiver table primary key when the
     relationship is identifier */
  void configureIndentifierRel(Table *dst_tab);

  /** @details Copy the columns from the reference table to the receiver table. The parameter not_null indicates
     that the columns must not accept null values */
  void copyColumns(Table *ref_tab, Table *recv_tab, bool not_null);

  /** @details This method is always executed before disconnection of the relationship.
  ...
  /// @details Creates the special primary key using the names stored in the 'column_ids_pk_rel' vector
  void createSpecialPrimaryKey(void);

  /// @details Removes the columns created by the relationship from the specified table primary key if exists.
  void removeColumnsFromTablePK(Table *table);

 protected:
  /// @details Destroy all the relationship attributes and constraints
  void destroyObjects(void);

  /// @details Returns all the refenced columns of the relationship
  vector<Column *> getGeneratedColumns(void);

  /// @details Returns the table generated by the n-n relationship
  Table *getGeneratedTable(void);

  /** @details Returns the generated foreign key that represents the 1-1, 1-n relationship,
 ...
 public:
  /// @details String used as the name suffix separator. Default '_'
  ...
  Relationship(unsigned rel_type,
   ...
   /// @details  Connects the relationship making the configuration according to its type
   void connectRelationship(void);

   /** @details Disconnects the relationship from the tables removing all the attributes / constraints
      deallocating all the created object. */
   void disconnectRelationship(bool rem_tab_objs=true);

   /// @details Defines the mandatory participation for the specified table
   void setMandatoryTable(unsigned table_id, bool value);

   /// @details Defines the automatic suffix generation for column names
   void setAutomaticSuffix(bool value);

   /// @details Returns the current state for automatic column name suffix generation
   bool isAutomaticSuffix(void);

   /** @details Warning: This method is effective only when the relationship is connected. Only
   ...
   /// @details Returns the column indexes used by the special primary key
   vector<unsigned> getSpecialPrimaryKeyCols(void);

   /// @details Defines the name of the auto generated table on the n-n relationship
   void setTableNameRelNN(const QString &name);

   /// @details Returns the name of the table auto generated by the n-n relationship
   QString getTableNameRelNN(void);

   /// @details Defines the suffix for the specified table id
   void setTableSuffix(unsigned table_id, const QString &suffix);

   /// @details Returns the suffix for the specified table id
   QString getTableSuffix(unsigned table_id);

   /// @details Defines if the created foreign key  is deferrable (only for 1-1, 1-n relationships)
   void setDeferrable(bool value);

   /// @details Returns if the created foreign key is deferrable (only for 1-1, 1-n relationships)
   bool isDeferrable(void);

   /// @details Defines the deferral type for the created foreign key (only for 1-1, 1-n relationships)
   void setDeferralType(DeferralType defer_type);

   /// @details Returns the deferral tyep for the created foreign key (only for 1-1, 1-n relationships)
   DeferralType getDeferralType(void);

   /// @details Defines if the relationship is identifier
   void setIdentifier(bool value);

   /// @details Returns if the relationship is identifier
   bool isIdentifier(void);

   /** @details Returns if the relationship is invalidated in relation to propagation of columns.
  ...
  /// @details Adds an attribute or constaint to the relationship.
  void addObject(TableObject *tab_obj, int obj_idx=-1);

  /// @details Returns one attribute or constraint using its name
  TableObject *getObject(const QString &name, ObjectType obj_type);

  /// @details Returns one attribute or constraint using its index
  TableObject *getObject(unsigned obj_idx, ObjectType obj_type);

  /// @details Removes on attribute or constraint using its index
  void removeObject(unsigned obj_id, ObjectType obj_type);

  /// @details Removes the specified attribute or constraint from the relationship
  void removeObject(TableObject *object);

  /// @details Removes all the attributes and constraints from the relationship
  void removeObjects(void);

  /// @details Remove one attribute using its index
  void removeAttribute(unsigned attrib_idx);

  /// @details Remove one constraint using its index
  void removeConstraint(unsigned constr_idx);

  /// @details Gets an attribute using its index
  Column *getAttribute(unsigned attrib_idx);

  /// @details Gets an attribute using its name
  Column *getAttribute(const QString &name);

  /// @details Gets an constraint using its index
  Constraint *getConstraint(unsigned constr_idx);

  /// @details Gets an constraint using its name
  Constraint *getConstraint(const QString &name); 

  /** @details Returns the index of a relationship attribute or constraint. Returns -1
     when the object doesn't exists */
  int getObjectIndex(TableObject *object);

  /// @details Returns the attribute count
  unsigned getAttributeCount(void);

  /// @details Returns the constraint count
  unsigned getConstraintCount(void);

  /// @details Returns the attribute or constraint count
  unsigned getObjectCount(ObjectType obj_type);

  /** @details Returns if the relationship has one or more identifier attributes.
  ...
  /** @details Returns table that receives a copy of the columns that represent the relationship
  ...
  /** @details Returns table which serves as a reference when coping the columns to the
     receiver table.
  ...
  /// @details Returns SQL / XML definition for the relationship.
  QString getCodeDefinition(unsigned def_type);

  /// @details Copies the attributes from one relationship to another
