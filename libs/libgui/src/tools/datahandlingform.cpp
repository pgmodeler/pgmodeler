...
DataHandlingForm::DataHandlingForm(QWidget * parent, Qt::WindowFlags f): QDialog(parent, f)
{
	...
	//truncate_menu.addAction(QIcon(GuiUtilsNs::getIconPath("truncate")), tr("Truncate"), QKeySequence("Ctrl+Del"), this, &DataHandlingForm::truncateTable)->setData(QVariant::fromValue<bool>(false));
	//truncate_menu.addAction(QIcon(GuiUtilsNs::getIconPath("trunccascade")), tr("Truncate cascade"), QKeySequence("Ctrl+Shift+Del"), this, &DataHandlingForm::truncateTable)->setData(QVariant::fromValue<bool>(true));
	...
	//connect(table_cmb, &QComboBox::currentIndexChanged, this, &DataHandlingForm::addDataGrid);
	connect(table_cmb, &QComboBox::activated, this, &DataHandlingForm::addDataGrid);

	//connect(table_cmb, &QComboBox::currentIndexChanged, this, __slot(this, DataHandlingForm::listColumns));
	//connect(table_cmb, &QComboBox::currentIndexChanged, this, __slot(this, DataHandlingForm::retrieveData));
...
void DataHandlingForm::addDataGrid()
{
	...
	DataGridWidget *data_grid_wgt = new DataGridWidget(schema_cmb->currentText(), table_cmb->currentText(),
																										 static_cast<ObjectType>(table_cmb->currentData().toUInt()),
																										 tmpl_conn_params);

	int tab_idx = data_grids_tbw->addTab(data_grid_wgt,
																			 table_cmb->itemIcon(table_cmb->currentIndex()),
																			 schema_cmb->currentText() + "." + table_cmb->currentText());
...
/* void DataHandlingForm::listColumns()
{
	Catalog catalog;
...
/* void DataHandlingForm::retrieveData()
{
	if(table_cmb->currentIndex() <= 0)
		return;
...
/* void DataHandlingForm::resetAdvancedControls()
{
...
/* void DataHandlingForm::loadDataFromCsv(bool load_from_clipboard, bool force_csv_parsing)
{
	try
	{
...
/* void DataHandlingForm::retrievePKColumns(const QString &schema, const QString &table)
{
	Catalog catalog;
	Connection conn=Connection(tmpl_conn_params);
...
/* void DataHandlingForm::retrieveFKColumns(const QString &schema, const QString &table)
{
	Catalog catalog;
	Connection conn=Connection(tmpl_conn_params);
...
/* void DataHandlingForm::markOperationOnRow(OperationId operation, int row)
{
...
/* void DataHandlingForm::addRow(bool focus_new_row)
{
...
/* void DataHandlingForm::removeNewRows(std::vector<int> ins_rows)
{
	if(!ins_rows.empty())
	{
...
/* void DataHandlingForm::clearChangedRows()
{
	changed_rows.clear();
...
/* void DataHandlingForm::browseTable(const QString &fk_name, bool browse_ref_tab)
{
...
/* void DataHandlingForm::browseReferrerTable()
{
	browseTable(qobject_cast<QAction *>(sender())->data().toString(), true);
} */

/* void DataHandlingForm::browseReferencedTable()
{
...
/* void DataHandlingForm::setColumnsCheckState(Qt::CheckState state)
{
	QListWidgetItem *item = nullptr;
...
/* void DataHandlingForm::truncateTable()
{
	try
	{
...
/* void DataHandlingForm::toggleColumnDisplay(QListWidgetItem *item)
{
	if(!item)
		return;
...
/* void DataHandlingForm::saveSelectedItems(bool csv_format)
{
	try
	{
