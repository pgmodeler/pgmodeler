...
ReferenceWidget::ReferenceWidget(QWidget *parent) : QWidget(parent)
{
	...
	connect(view_def_chk, &QCheckBox::toggled, this, [&](bool checked){
	...
	connect(ref_object_sel, &ObjectSelectorWidget::s_objectSelected, this, [&](){
		col_alias_edt->setEnabled(dynamic_cast<Column *>(ref_object_sel->getSelectedObject()));
	});

	connect(ref_object_sel, &ObjectSelectorWidget::s_selectorCleared, this, [&](){
		col_alias_edt->setEnabled(false);
	});

	connect(ref_table_sel, &ObjectSelectorWidget::s_selectorChanged, this, [&](bool selected){
