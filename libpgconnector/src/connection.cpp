...
QString Connection::getConnectionId(bool host_port_only)
{
	QString addr;
	...
	if(host_port_only)
		return(QString("%1:%3").arg(addr, connection_params[PARAM_PORT]));
	else
		return(QString("%1 (%2:%3)").arg(connection_params[PARAM_ALIAS], addr, connection_params[PARAM_PORT]));
}
