...
void Connection::setConnectionParam(const QString &param, const QString &value)
{
	...
	if(param.isEmpty())
		throw Exception(AsgInvalidConnParameter, __PRETTY_FUNCTION__, __FILE__, __LINE__);
...
void Connection::validateConnectionStatus(void)
{
	if(cmd_exec_timeout > 0)
	{
		...
		{
			close();
			throw Exception(ConnectionTimeout, __PRETTY_FUNCTION__, __FILE__, __LINE__);
		}
	}
...
void Connection::connect(void)
{
	...
	if(connection_str.isEmpty())
		throw Exception(ConnectionNotConfigured, __PRETTY_FUNCTION__, __FILE__, __LINE__);
	else if(connection)
	{
		if(!silence_conn_err)
			throw Exception(ConnectionAlreadyStablished, __PRETTY_FUNCTION__, __FILE__, __LINE__);
		else
		{
	...
	if(connection==nullptr || PQstatus(connection)==CONNECTION_BAD)
	{
		...
		throw Exception(QString(Exception::getErrorMessage(ConnectionNotStablished))
						.arg(PQerrorMessage(connection)), ConnectionNotStablished,
...
void Connection::executeDMLCommand(const QString &sql, ResultSet &result)
{
	...
	{
		throw Exception(QString(Exception::getErrorMessage(SQLCommandNotExecuted))
						.arg(PQerrorMessage(connection)),
						SQLCommandNotExecuted, __PRETTY_FUNCTION__, __FILE__, __LINE__, nullptr,
...
void Connection::executeDDLCommand(const QString &sql)
{
	...
	if(strlen(PQerrorMessage(connection)) > 0)
	{
		...
		throw Exception(QString(Exception::getErrorMessage(SQLCommandNotExecuted))
						.arg(PQerrorMessage(connection)),
						SQLCommandNotExecuted, __PRETTY_FUNCTION__, __FILE__, __LINE__, nullptr,	field);
	}
