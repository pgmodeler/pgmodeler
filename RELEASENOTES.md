...
<strong>Summary:</strong> dispite the desigation <em>alpha1</em> this release brings important improvements on stability and speed being more reliable than the current stable release <strong>0.7.2</strong>. The key features of this version are: a new form that permits user to handle table's data, several improvements on SQL tool, improved validation, improved code generation speed, improved startup speed and many others. Update <strong>HIGHLY RECOMMENDED!</strong><br/>
<strong>NOTE:</strong><em> models generated in previous versions will demand the use of fix model feature!</em>.
...
* [New] Source code dialog now optionally appends the SQL of permissions of the current object.
* [New] Added an option to source code privew dialog to include table's children objects' SQL.
* [New] Added a field on all object's editing form to expose the object's internal id.
* [New] Added an option on the database import process to generate random colors to relationships.
* [New] The model objects widget gained a filter field that is capable to filter objects by their name or internal id.
...
* [Change] Minor update on shortcuts and tooltips of bottom control bar buttons at main window.
...
* [Change] Message box objects are now used over demand and no more as class attributes.
* [Change] The software startup was hugely improved by removing singleton dialogs from main window's constructor, now they are used over demand.
...
* [Change] Minor fix on object's schema files to correctly generate the appended/prepended code.
* [Change] Major improvement on all core classes (resposible to represent database objects) in order to keep the their cache syncronized with the current configuration.
...
* [Fix] Minor fix on message box instances to resize according to the height of message.
...
* [Fix] Minor fix on BaseRelationship and Relationship classes to correctly invalidate envolved object's when connect or disconnect.
* [Fix] Fixed a crash when closing the last model and the object finder was visible and with result list filled.
